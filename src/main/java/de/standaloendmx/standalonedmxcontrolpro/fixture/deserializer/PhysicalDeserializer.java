...
public class PhysicalDeserializer implements JsonDeserializer<FixturePhysical> {
    ...
    public FixturePhysical deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context) throws JsonParseException {
        ...
        if (jsonObject.getAsJsonObject("bulb") != null) {
            if (jsonObject.getAsJsonObject("bulb").get("colorTemperature") != null)
                colorTemp = jsonObject.getAsJsonObject("bulb").get("colorTemperature").getAsInt();
            if (jsonObject.getAsJsonObject("bulb").get("lumens") != null)
                lumen = jsonObject.getAsJsonObject("bulb").get("lumens").getAsInt();
        ...
        if (data != null)
            dim = new FixtureDimension(data.get(0).getAsDouble(), data.get(1).getAsDouble(), data.get(2).getAsDouble());
